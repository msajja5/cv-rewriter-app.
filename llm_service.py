...
async def generate_ai_response_with_llm_stream(
    ...
    # 1. Identity & Framing
    system_prompt = "You are Manjunath Sajjan speaking in a live job interview. You read answers directly on screen. Write EXACTLY what he would say — in his voice, grounded in his real career history.\n\n"

    # 2. Target role context
    system_prompt += f"ROLE BEING INTERVIEWED FOR: {job_role}\n\n"

    # 3. Question-type routing
    intent = detect_question_routing(question)
    system_prompt += f"QUESTION TYPE DETECTED: {intent}\n\n"

    # 4. Full Candidate Profile
    system_prompt += f"CANDIDATE PROFILE:\n{MANJUNATH_PROFILE}\n\n"

    # 5. Answer format rules
    system_prompt += """RULES:
- DO NOT just talk about Ontex for every answer. Look at the whole profile (QuEST, Nike, Solvay) and pick the most relevant specific company.

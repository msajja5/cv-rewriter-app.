...
async def generate_ai_response_with_llm(question: str, cv: str, job_role: str, context: list) -> Tuple[str, str]:
    ...
    if gemini_key:
        try:
            ...
            # Remove last message from history, pass it to generate_content
            # If the history only has the first user message (which now contains the system prompt + the question),
            # we need to ensure we don't completely clear the history if it's the only thing there.
            # Actually, the last message in `messages` is already the question, which is what we just appended to `gemini_history`.
            # We should remove it from `gemini_history` because `chat.send_message` will append it.
            # Wait, if `gemini_history` only has 1 item, it's the question + system prompt.
            # We can just start the chat with empty history, and send the system prompt + question as the first message.
